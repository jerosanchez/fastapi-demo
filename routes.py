...
from mappers import to_post_response
...
@router.get("/posts")
async def get_posts(db: Session = Depends(get_db)) -> dict[str, list[PostResponse]]:
    posts = db.query(models.Post).all()
    return {"data": [to_post_response(post) for post in posts]}


@router.post("/posts", status_code=status.HTTP_201_CREATED)
async def create_post(
    post_data: PostCreate, db: Session = Depends(get_db)
) -> dict[str, PostResponse]:
    new_post = models.Post(**post_data.model_dump())
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return {"data": to_post_response(new_post)}
...
async def update_post(
    ...
    db.commit()
    db.refresh(post)
    return {"data": to_post_response(post)}
